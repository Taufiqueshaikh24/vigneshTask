...
export async function sendEmail(to, type, name, content) {
  ...
  try {
    if (type === "verification") {
      ...
      subject = "Verify Your Email - LockMyFile";
      htmlContent = `
        <html>
          <body style="text-align:center; font-family: Arial, sans-serif; background-color: #f4f4f4; padding: 20px;">
            <div style="max-width: 600px; margin: auto; background: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
              ...
              <h3 style="color:#007bff; background:#e7f3ff; display:inline-block; padding:10px 20px; border-radius:5px;">${content}</h3>
              <p style="color:#777;">If you didn't request this, ignore this email.</p>
              <p style="color:#888;">Best regards,<br/>LockMyFile Team</p>
            </div>
          </body>
        </html>`;
    } else if (type === "reset") {
      // 🔹 Password Reset Template
      subject = "Reset Your Password - LockMyFile";
      htmlContent = `
        <html>
          <body style="text-align:center; font-family: Arial, sans-serif; background-color: #f4f4f4; padding: 20px;">
            <div style="max-width: 600px; margin: auto; background: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
      ...
      // 🔹 Login OTP Template
      subject = "Login OTP - LockMyFile";
      htmlContent = `
        <html>
          <body style="text-align:center; font-family: Arial, sans-serif; background-color: #f4f4f4; padding: 20px;">
            <div style="max-width: 600px; margin: auto; background: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
              <h2 style="color:#28a745;">Login OTP</h2>
              <p style="color:#555;">Hello <strong>${name}</strong>,</p>
              <p>Use the following OTP to log into your account:</p>
              <h3 style="color:#28a745; background:#eaffea; display:inline-block; padding:10px 20px; border-radius:5px;">${content}</h3>
              <p style="color:#777;">This OTP is valid for a limited time. If you didn't request this, please ignore it.</p>
              <p style="color:#888;">Best regards,<br/>LockMyFile Team</p>
            </div>
          </body>
        </html>`;
    ...
    // ✅ Send Email
    await transporter.sendMail({
      from: `LockMyFile <${process.env.EMAIL_USER}>`,
      to,
      subject,
      html: htmlContent,
    });
